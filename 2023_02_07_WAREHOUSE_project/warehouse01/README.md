...
3. A menu is printed out showing 3 options: `1. List items by warehouse`, `2. Search an item and place an order` and `3. Quit`.
4. The user is asked to pick a choice using the numeric values associated.
    4.1. If the user picked `1`, the script should print each of the items in each of the warehouses (first all items from a warehouse and then all items from the other). Then it should stop execution.
    4.2 If the user picked `2`, the script should:
        a. Ask the user to input an item name and search the warehouses.
        b. The script will print:
            a. The total amount of items in any warehouse that match that name.
            b. The location of those items: the name of the warehouse (ex: `Warehouse 1`), if it can only be found in one, `Both warehouses` if it is in both and `Not in stock` if it is in none.
            c. If it can be found in more than one warehouse, it will also print a line saying which warehouse has the highest amount of those items (and how many does it have).
        c. Ask the user if they want to place an order for this item.
            a. If the answer is no, it will do nothing else.
            b. If the answer is yes, it should ask the user how many do they want.
                a. If the desired amount is equal or lower than the total available, it will proceed and show a message saying the order has been placed. *The message should show the item name and amount ordered*.
                b. If the desired amount is higher than the total available, it should show an error message and should ask the user if they want to order the maximum available, instead.
                    a. If they answer yes, it will proceed and show a message saying the order has been placed. *The message should show the item name and amount ordered*.
                    b. If they answer no, it will do nothing else.
    c. If the user picked `3`, the script should do nothing else.
    d. If the user types anything different than `1`, `2` or `3` it should show an error message indicating the operation entered is not valid.
5. Display a final message thanking the user for the visit, using their name typed at the beginning.
